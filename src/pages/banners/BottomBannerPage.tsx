...
const BottomBannerPage: React.FC = () => {
  ...
  const fetchBanners = async () => {
    ...
    try {
      const response = await BannerApiService.getBottomBanners();
  ...
  const handleEditBanner = (banner: Banner) => {
    ...
    const formattedStartDate = banner.startDate
      ? (() => {
          ...
          console.log("원본 시작일(하단):", banner.startDate, "변환된 시작일:", formattedDate);
    ...
    const formattedEndDate = banner.endDate
      ? (() => {
          ...
          const minutes = String(date.getMinutes()).padStart(2, "0");
          const formattedDate = `${year}-${month}-${day}T${hours}:${minutes}`;
          console.log("원본 종료일(하단):", banner.endDate, "변환된 종료일:", formattedDate);
          return formattedDate;
        })()
      : "";
      ...
      if (isEditing && currentBanner.id) {
        ...
        try {
          ...
          try {
            ...
            console.error("수정 후 데이터 확인 중 오류(하단):", checkErr);
          }
