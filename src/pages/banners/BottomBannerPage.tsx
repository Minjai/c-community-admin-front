...
const BottomBannerPage: React.FC = () => {
  ...
  const fetchBanners = async (page: number = 1, limit: number = 10, searchValue: string = "") => {
    ...
    try {
      const response = await BannerApiService.getBottomBanners(page, limit, searchValue);
  ...
  const handleBulkPositionSave = async () => {
    ...
    try {
      ...
      await Promise.all(
        changed.map((banner) =>
          BannerApiService.updateBottomBanner(banner.id, {
            id: banner.id,
      ...
      {currentBanner && (
        ...
        >
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">버튼 색상</label>
              <div className="flex items-center space-x-2">
                <input
                  type="text"
                  value={currentBanner.buttonColor || "#000000"}
                ...
                <input
                  ...
                  value={currentBanner.buttonColor || "#000000"}
                  onChange={(e) => handleInputChange("buttonColor", e.target.value)}
                  className="h-9 w-9 p-0 rounded-md border-gray-300 cursor-pointer"
                />
              </div>
            </div>
          </div>
