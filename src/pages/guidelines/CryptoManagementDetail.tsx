...
const CryptoManagementDetail = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode) {
        // 기존 가이드라인 수정
        const response = await axios.put<Post>(`/crypto-guideline/${id}`, {
          title,
          content,
        ...
        if (response.status === 200) {
          ...
          navigate("/guidelines/crypto");
        } else {
          setError("가이드라인 수정 중 오류가 발생했습니다.");
        }
      } else {
        // 새 가이드라인 작성
        const response = await axios.post<Post>("/crypto-guideline", {
          title,
          content,
  ...
  const getPostDetail = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get<Post>(`/crypto-guideline/${id}`);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end space-x-3">
          <button
            type="button"
            onClick={() => navigate("/guidelines/crypto")}
...
export default CryptoManagementDetail; 